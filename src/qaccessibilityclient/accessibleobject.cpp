...
AccessibleObject::AccessibleObject(RegistryPrivate *registryPrivate, const QString &service, const QString &path, AccessibleObject *parent)
    : QObject(parent)
    , d(new AccessibleObjectPrivate(registryPrivate, service, path))
{
    ...
    if (registryPrivate->m_cache) {
        ...
        Q_ASSERT(registryPrivate->m_cache->get(id) == 0);
        registryPrivate->m_cache->add(id, this);
    }
}
...
QString AccessibleObject::uniqueId() const
{
...
{
    Q_ASSERT(d);
    // FIXME: rather check that this object is still in the cache
...
AccessibleObject *AccessibleObject::accessibleParent() const
{
    return d->registryPrivate->accessibleParent(this);
}

QVector<AccessibleObject *> AccessibleObject::children() const
{
    return d->registryPrivate->children(this);
}

//QVector<QVector<AccessibleObject *> > AccessibleObject::children(const QList<Role> &roles) const
//{
//    QVector< QVector<AccessibleObject *> > result(roles.count());
//    QVector<AccessibleObject *> all = children();
//    for(int i = 0; i < all.count(); ++i) {
//        AccessibleObject *child = all[i];
...
QPoint AccessibleObject::focusPoint() const
{
    ...
    AccessibleObject *p = qobject_cast<AccessibleObject *>(parent());
    if (p && p->isValid())
...
QVector<AccessibleObject *> AccessibleObject::selection() const
{
...
QACCESSIBILITYCLIENT_EXPORT QDebug QAccessibleClient::operator<<(QDebug d, const AccessibleObject *const object)
{
    d.nospace();
